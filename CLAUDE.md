...
| `consolidate.md` | live | **Built 2026-09-15, untested on the testnet**: `proxi node consolidate`, a permanent wallet process that sweeps the account's sigLock outputs and reclaimable tag-alongs and moves what is above a configured minimum to a sequencer (`send_to_sequencer`), into a delegation (`autodelegate`) or into one output. Written independently of `proxi node mine`, whose treasury loop is retired in a later step once consolidate is tested. Read before touching either, and for the trigger and split rules (balance over `threshold_prox` on at least two outputs, or `compact_at` outputs; unconsumed outputs count toward the minimum; remainders under the storage floor folded). |
