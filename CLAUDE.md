...
**The working set.** Eleven documents: the two hard constraints, six live
specs, one spec not yet implemented, and two take 1 documents that live on `develop-take1` only, the todo of
breaking changes and the top-up spec. Nothing is queued on `develop`, and there
is no longer a meta tracker — the documentation effort closed on 2026-09-03.
Audited against `develop` and a running node the same day.
...
| `consolidate.md` | live | **Built 2026-09-15, delegation mode redesigned and verified 2026-09-19** (`31fb6003`; on loc0 and on a local standalone node: create, askstop-and-top-up, fold, stale re-delegation): `proxi node consolidate`, a permanent wallet process that sweeps the account's sigLock outputs and reclaimable tag-alongs and moves what is above a configured minimum to a sequencer (`send_to_sequencer`), into a delegation (`autodelegate`) or into one output. Delegation is a market mode: the wallet is a price taker (requires exactly the cut its target leaves; `delegate.minimum_cut` not read), a `random` target is drawn by the rating of `sequencer_rating.md`, the set is driven by `target_delegations` (5) and `target_delegation_prox` (10,000), and every tick opens by tidying the spendable delegations: fold above the count, re-delegate one whose target is inactive, refuses it as loss-making, is not the pinned one, or left it unfrozen for an epoch. `max_delegations` is read as the older name. On `develop-take1` the miner's treasury loop is retired and `proxi node mine` only mines; the consolidator on the same profile puts the payouts to work (`--disable_consolidation` is accepted and ignored). Read before touching either, and for the trigger and split rules (balance over `threshold_prox` on at least two outputs, or `compact_at` outputs; unconsumed outputs count toward the minimum; remainders under the storage floor folded). |
| `mine_vrf_pow.md` | live | **Built** (`6d217554`, hardfork, network reset 2026-09-08): the mine chain's proof of work is the trailing-zero count of an ECVRF output under the miner's key over predecessor ID, slot and nonce, with the 80-byte proof and the nonce in the mine input's lock unlock parameters. It replaced the signed-transaction hash, whose free signature nonce let an attempt cost a point addition instead of a scalar multiplication. Read for why the K bits must be on the VRF output and not on any hash containing a signature or a proof, and for the review of the in-house `util/vrf`, which the branch inflation bonus depends on too. Does not and cannot fix the GPU advantage. Moves to the archive once the fresh network has confirmed transits. |
| `sequencer_rating.md` | live | **Built 2026-09-20**: how `proxi` picks a `random` delegation or tag-along target. Weighted rank sum over criteria, the price criterion counted twice (delegation: share left desc x2, balance desc, frozen-to-balance ratio asc; tag-along: minimum fee asc x2, balance desc), the list sorted by rating and drawn with weight linear in position; active = sequencer output in the LRB state at most 5 slots old; no price floor, only `proxi node delegate` drops targets leaving less than its `--cut`. Pure functions in `ledger/txbuildercore/seq_rating.go`, one fetch in `proxi/glb/sequencers.go`, used by consolidate, delegate and the `random` tag-along target; `proxi node seq_rating [--tag_along]` prints the table. The miner's own picker stays until its treasury loop is retired. Read before touching any target selection. |
| `take1_todo.md` | take 1 todo | **`develop-take1` only.** The list of breaking changes that ship with the next network reset, each linked to its spec where one exists: ensure constraints on requests step aside at `constTagAlongSlots` (done), the top-up request (done), pace 1 with the take 1 emission schedule and the fixed 1 PROX mine fee (done), and the node, miner and wallet work that can land before the reset. Operational items stay outside the repository. Read first when working on the branch. |
| `delegation_topup.md` | take 1 spec | **Built on `develop-take1` 2026-09-22 (hardfork), miner's treasury loop retired the same day.** One tag-along to the target adds tokens to a delegation in any state the target can act on, and a frozen delegation stays frozen at the share pinned when the freeze began. No fee: the request balance is the amount, the sequencer is paid in frozen coverage and enforces a minimum top-up (`MinTopUp` in its data, floor 100 PROX). New `ensureTopUpDelegation` guarantees delivery; the delegate lock's target path, with a third unlock byte naming the request, enforces the exact balance including the advance on the newly frozen amount. Sequencer request code 4; consolidator placement without askstop; miner treasury loop retired. Carries the reclaim fix: both ensure constraints step aside at `constTagAlongSlots` so the sender can reclaim a request in its exclusive window, and the wallet classifier recognises request outputs. Read before touching delegation transitions, sequencer requests or the consolidator's placement. |
| `mine_conflict_rule.md` | live | Two parts. **Part A, built `5aac046b`**, policy only: among competing mine transits on one predecessor the canonical winner is the older successor slot, then the smaller VRF output, then the txid; the miner tree (`proxi/node_cmd/mine_tree.go`) and the sequencer (`sequencer/task/mine_transits.go`, which holds a transit until the settlement window at the end of its slot) apply the same order, so inclusion order no longer decides. **Part B, built on `develop-take1` 2026-09-23 for the next reset**: pace 1 with an asymmetric retarget on a full-slot counter carried on the mine output, cap 56, the take 1 schedule (95 PROX, 60 days, then linear) and a fixed 1 PROX transit fee; the miner grinds a contested slot for a smaller VRF output until the settlement tick. Neither changes expected shares: a contest goes to a draw nobody can choose, and the share of contests won is the share of hashrate. Read before touching the miner's tie-break or the sequencer's tag-along ordering. |
| `control_succession.md` | spec, not built | Safe sequencer succession: a two-stage grant/claim handshake that moves control of a sequencer chain to a new holder ID without risking a bricked chain. The controller grants a successor in the on-chain seq-data, the successor claims from a wallet holding its key, and the handoff milestone flips the chain output's sigLock. No ledger change, no hardfork: the work is sequencer request codes 5 and 6, a predecessor lock gate in the milestone builder, an LRB-based give-up in the sequencer loop, a waiting mode for the successor node and `proxi node seq grant|claim`. **Reviewed against `develop` 2026-09-23, nothing implemented.** Read before touching sequencer request handling, the start-up controller check or the sequencer loop's stop conditions. |
...
- **EasyFL builtins must be pure — no side effects.** An `evalXxx` builtin computes a result from the tx context and returns it; it must not mutate per-tx accumulator state as a side effect of being evaluated. EasyFL evaluation is lazy (`if(cond, a, b)` evaluates only one arm) and position-dependent, so a builtin that accumulates when it runs can be made to count on one side only by hiding it behind a conditional — the native-token counterfeit found in the 2026-09 audit. When an aggregation across outputs is genuinely needed, drive it from a **structural scan** of the produced/consumed tuples (iterate the outputs, inspect each constraint element's root call) at tx level, not from the builtin's evaluation. The builtin, if it stays, is a pure predicate that validates its own shape.
...
- **No AI attribution or co-authoring anywhere Claude produces output.** This
  covers commit messages, PR descriptions, code comments, file headers, docs,
  and any other artifact: no `Co-Authored-By: Claude…`, no `Claude-Session:`
  trailer, no "Generated by Claude Code", no "🤖 Generated with…", no
