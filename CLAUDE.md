...
**The working set.** Nine documents: the two hard constraints, six live
specs, and one take 1 spec that lives on `develop-take1` only. Nothing is
queued, and there is no longer a meta tracker — the documentation effort closed
on 2026-09-03. Audited against `develop` and a running node the same day.
