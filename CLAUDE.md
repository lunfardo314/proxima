...
| `take1_todo.md` | take 1 todo | **`develop-take1` only.** The list of breaking changes that ship with the next network reset, each linked to its spec where one exists: ensure constraints on requests step aside at `constTagAlongSlots` so the sender can reclaim from slot 30 (done on the branch), the top-up request, pace 1, the take 1 emission schedule, the 1% mine fee question, and the node, miner and wallet work that can land before the reset. Operational items stay outside the repository. Read first when working on the branch. |
