...
| `claude/sequencer_local_optimum.md` | proposal, not implemented | Why the skeleton search gets stuck: sibling branches conflict on the parent's stem, so the landscape is disconnected basins and the first-fit seed alone picks one. Reverting own state is inside the search space and is the defence against tag-along conflict attacks. Proposes several factories with different heuristics (one randomised) sharing a checked-combination set, scored by coverage plus newly consolidated sequencers. Extends the single-factory TSF design in `claude/sequencer.md`. |
