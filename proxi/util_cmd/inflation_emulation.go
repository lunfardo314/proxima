...
func runInflationEmulationCmd(cmd *cobra.Command, args []string) {
	...
	fmt.Printf("Initial supply: %s tokens (%s PRXI)\n", util.Th(lib.InitialSupply), util.Th(lib.InitialSupply/ledger.PRXI))
	...
	fmt.Printf("  Proforma supply:   %s tokens (%s PRXI)\n", util.Th(finalSupply), util.Th(finalSupply/ledger.PRXI))
	fmt.Printf("  Total inflated:    %s tokens (%s PRXI)\n", util.Th(finalSupply-lib.InitialSupply), util.Th((finalSupply-lib.InitialSupply)/ledger.PRXI))
