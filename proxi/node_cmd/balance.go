...
To see what of it can be CONSUMED rather than what it is worth, use
'proxi node compact scan'.`,
		Args: cobra.NoArgs,
		Run:  runBalanceCmd,
	}
	...
	// Read straight off the bound variable — no viper binding: 'lrb-depth' is
	// registered by the compaction scan too, and a viper key bound from two
	// command trees keeps only the last registration.
