...
func runSyncInfoCmd(_ *cobra.Command, _ []string) {
	...
	sorted := util.KeysSorted(syncInfo.PerSequencer, func(k1, k2 core.ChainID) bool {
		return bytes.Compare(k1[:], k2[:]) < 0
	})
	for _, seqID := range sorted {
		si := syncInfo.PerSequencer[seqID]
		seenBack := time.Since(core.MustNewLogicalTime(core.TimeSlot(si.LatestSeenSlot), 0).Time())
		bookedBack := time.Since(core.MustNewLogicalTime(core.TimeSlot(si.LatestBookedSlot), 0).Time())
		active := seenBack < core.TimeSlotDuration()
