...
func (srv *server) registerHandlers() {
	...
	// See kb/archive/shipped/wallet_eval_api.md.
	srv.addHandler(api.PathEval, srv.eval)
	// Unified state-query endpoint. See kb/archive/shipped/get_outputs.md.
...
// parses. See kb/archive/shipped/wallet_eval_api.md.
...
func (srv *server) getMainChain(w http.ResponseWriter, r *http.Request) {
	...
	main, err := multistate.GetMainChain(srv.StateStore(), maxDepth)
	if err != nil {
		api.WriteErr(w, err.Error())
		return
	}
...
// kb/archive/shipped/get_outputs.md. Single mandatory parameter `index_value`
...
func (srv *server) getOutputs(w http.ResponseWriter, r *http.Request) {
	...
	// reclaim-window). See kb/archive/shipped/wallet_eval_api.md (Phase C3').
