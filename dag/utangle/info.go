...
func (ut *UTXOTangle) InfoLines(verbose ...bool) *lines.Lines {
	...
	for _, e := range slots {
		...
		for _, vid := range branches {
			...
			util.Assertf(isAvailable, "sequencer ID expected in %s", vid.IDShortString())
			ln.Add("    branch %s, seqID: %s, coverage: %s", vid.IDShortString(), seqID.Short(), util.GoThousands(coverage))
			if verb {
