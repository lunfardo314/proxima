...
func (d *DAG) VerticesInSlotAndAfter(slot core.TimeSlot) []*vertex.WrappedTx {
...
func (d *DAG) LinesVerticesInSlotAndAfter(slot core.TimeSlot) *lines.Lines {
	return vertex.VerticesLines(d.VerticesInSlotAndAfter(slot))
}

func (d *DAG) _timeSlotsOrdered(descOrder ...bool) []core.TimeSlot {
	desc := false
	if len(descOrder) > 0 {
		desc = descOrder[0]
	}
