...
func TestAttachConflicts1Attacher(t *testing.T) {
	...
	t.Run("n double spends consumed", func(t *testing.T) {
		...
		if nConflicts > 1 {
			...
			util.RequireErrorWith(t, vid.GetError(), "conflict", "in the past cone", testData.forkOutput.IDShort())
		} else {
	...
	t.Run("conflicting tx consumed", func(t *testing.T) {
		...
		td := txbuilder.NewTransferData(testData.privKey, testData.addr, base.MaximumTime(inTS...).AddTicks(ledger.TransactionPace()))
...
func TestAttachConflictsNAttachersOneFork(t *testing.T) {
	...
	util.RequireErrorWith(t, vidSeq.GetError(), conflict.StringShort(), "conflict")
	//testData.wrk.SaveGraph("utangle")
}
...
func TestAttachConflictsNAttachersOneForkBranchesConflict(t *testing.T) {
	...
	util.RequireErrorWith(t, vid.GetError(), "conflicting branch endorsement")
}
