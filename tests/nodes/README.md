...
## Run more nodes in 'access node' mode 
...
The command line prefix `proxi node` means the `proxi` is accessing the ledger via the node's API, unlike 
in commands staring with `proxi db`, which accesses the DB directly.

By running command `proxi node transfer 1000 -t "addressED25519(0xaa401c8c6a9deacf479ab2209c07c01a27bd1eeecf0d7eaa4180b8049c6190d0)"`
will produce transfer transaction from the address, controlled by private key in `0/proxi.yam` to the corresponding address.
...
```
which means `1000` tokens gone to the target address plus `500` tokens consumed by the sequencer as the _tag-along_ fee.

The command `proxi node balance` in the directory `1` will output with those `1000` tokens:
```text
Command line: 'proxi node balance'
using profile: ./proxi.yaml
...
private key and tag-along sequencer is configured the bootstrap sequencer `af7bedde1fea222230b82d63d5b665ac75afbe4ad3f75999bb3386cf994a6963`, 
the only one currently running.
...
So, we need to split the whole supply, controlled by the bootstrap sequencer on its chain, into 5 pieces. This we do by running
...
The balance now looks like that:
```text
Command line: 'proxi node balance'
using profile: ./proxi.yaml
using API endpoint: http://127.0.0.1:8000
...
```
Now the following commands distribute all tokens other 4 addresses, `200000000000000` tokens to each:
...
the current working and run the following command:
...
into the `sequencers.seq1.sequencer_id` key in the `proxima.yaml`. The enable sequencer by putting `true` into
`sequencers.seq1.enable`.

The restart node with `CTRL-C` and command `proxima` again.
You will see sequencer `seq1` starting on the node `1`. The two nodes will be exchanging sequencer transactions, 
and you will see both of them on logs of both nodes.
...
All 5 sequencers `boot`, `seq1`, `seq2`, `seq3`, `seq4` should be exchanging transactions and thus coming to the consensus on one single
chain. 

If some of does not appear, stopping and starting node again usually helps. Remember: this is a prototype (very early alpha) 
version of the node!
