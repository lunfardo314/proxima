...
| `big_tasks.md` | 2026-02-07 | A top-level task checklist, part struck through as done. | Overtaken by the `kb/TODO.md` backlog, itself deleted as stale in 2026-08. Kept only as a record of what the roadmap looked like in February. |
...
| `ratecontrol2.md` | 2026-04-07 | memDAG pruning/GC architecture, gated on whether a vertex is referenced by the sequencer. | Superseded. Pruning is now governed by `kb/dag_semantics.md` §2.5, whose criterion is "rooted and not consumed by a not-rooted transaction" — explicitly *not* a TTL or reference test. |
...
touching that area — and `kb/dag_semantics.md`'s M1: pruning and GC are
