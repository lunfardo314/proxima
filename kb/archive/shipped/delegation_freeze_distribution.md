...
> **SHIPPED** — the amount-weighted balancer that spreads delegation freeze
> epochs across the reachable window, instead of piling them onto one epoch.
> On `develop`: `sequencer/delegationpool/delegationpool.go` and
> `selectDelegationsToFreeze` in `sequencer/task/proposal.go`, covered by
> `sequencer/task/freeze_epoch_test.go`. **The code is the truth.** Archived
> 2026-09-03.
>
> Read it for what the code cannot record: the **rejected hash-based
> alternative** (assign the epoch by `hash(chainID) mod N` — stateless and
> pool-free, but amount-blind, so a few large delegations landing on one epoch
> reproduce the coverage cliff), and the reasoning behind the 2026-07-06 change
> that made continuations run the same balancer as first-time freezes. The
> earlier continuation-to-max rule collapsed every delegation of a target onto
> one epoch whenever a network restart re-froze them together, and never
> recovered.
>
> Section 10 keeps three **design sign-offs that were never formally closed** —
> the cache-scope exception, the new-delegation backlog TTL, and the full-output
> fetch at freeze time. All three describe behaviour that is already built;
> verify against the code before treating any of them as an open decision.
