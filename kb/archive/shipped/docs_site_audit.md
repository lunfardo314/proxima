...
> and the reasoning behind the deletions. The live tracker is `kb/archive/shipped/docs.md`.
...
- Companion: repo-docs tracker `kb/archive/shipped/docs.md` (Phase 1, complete).
...
2. **Native tokens and foundries** — `foundry(supply)` chained foundry, `token(...)`
   ...
   across the tx. New `txdocs/` page. Spec: `kb/archive/shipped/native_token.md`; memory
...
4. **Expand the `participate/` section** — ✅ **DONE (2026-06-09).** All 8 user-facing
   ...
   deleted — see `kb/archive/shipped/kb_reorg.md`.)*
...
- 2026-06-03 — Started site editing on new branch **`ver8`** (based on `ver7`, pushed to
  ...
  Site edits land on `ver8`; the proxima `kb/` tracker stays on `develop`.
