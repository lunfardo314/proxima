# docs.md — the documentation effort: plan, status, progress

> **SHIPPED** — the tracker the 2026 documentation effort was run from, closed
> and archived 2026-09-03 when the last queue row was retired. Items 1 to 5 were
> completed; item 6, a maintenance-guidelines document, was deliberately dropped
> in favour of re-auditing the docs site page by page, which supersedes a
> guidelines document written in advance of it. It absorbed the former
> `docs_site_audit.md`, archived separately.
>
> **Its working principles were rescued into `CLAUDE.md`** ("Writing
> documentation", under Working Rules) and are the live copy — do not read them
> from here.
>
> **Two loose ends were still open when this closed**, both re-verified
> 2026-09-03 and neither a documentation bug:
>
> - `ledger/txbuildercore/tx_layout.go` calls `TxSequencerDataBytes` "4-byte
>   sequencer info"; `SequencerDataLen` is 2.
> - `ledger/def/chain.easyfl` says the chain constraint sits at index 2 in three
>   comments; the authoritative value is `ConstraintIndexChain` = 3. The comments
>   are free to fix — comment text is not hashed into the LibraryHash — but the
>   error literal `!!!chain_constraint_must_be_at_index_2` is **not**: `!!!` text
>   compiles into the bytecode, so renaming it is a hardfork. A fourth instance
>   sits in `ledger/tests/claude_index_bounds_test.go`.
>
> A third loose end, `core/attacher/README.md` opening with `TODO needs review`,
> was already fixed before this closed; this tracker never caught up, which is
> its own argument for retiring it.
...
Tracked as Phase 6 of `kb/archive/shipped/kb_reorg.md` (archived 2026-09-03), not here — it is the last phase of that
plan and consumes the two remaining `QUEUED` documents, `kb/inflation.md`
and `kb/research/tick_duration.md`.
...
with a line on when to read it, the docs-site sections, the `kb/` working
...
### 4. `kb/TODO.md` — DELETED 2026-08-26
...
- **2026-08-25** — Five items scheduled into the pending queue at user
  ...
  orientation with all references, a review of `kb/TODO.md` (which ended in
