# UTXO indexing refactor — full spec

> **SHIPPED** — the complete breaking refactor: problem, goal, per-file code
> analysis, migration plan by phase with commits, risks, and three related
> cleanups (user-facing lock syntax, human-readable transaction dumps, dropping
> unused unlock-parameter slots). Archived from
> `ledger/multistate/utxo_indexing.md` on 2026-08-26, when that file was reduced
> to the reference a developer actually needs at the code.
>
> The code is the truth. Read this for the *why*: why index keys were moved out
> of Go `Lock` methods into the UTXO tuple itself, and what was considered and
> rejected on the way.
