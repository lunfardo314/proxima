...
| `delegate_lock.md` | 2026-03-07 | `delegateLock`'s master argument changed from accountable bytecode to a raw holder ID. | `ledger/def/lock_delegate.easyfl`. Much evolved since — see `kb/research/delegation_scalability.md`. |
...
| `delegation_freeze_distribution.md` | 2026-07-06 | The amount-weighted balancer spreading delegation freeze epochs across the reachable window, so they do not pile onto one epoch and cut a coverage cliff. | `sequencer/delegationpool/`, `selectDelegationsToFreeze` in `sequencer/task/proposal.go`, `sequencer/task/freeze_epoch_test.go`. Records the **rejected hash-based** assignment and three never-closed design sign-offs. |
...
| `docs.md` | 2026-08-26 | The tracker the documentation effort was run from: where each kind of document lives, the working principles, and the six-item queue. | Items 1-5 done; item 6 (maintenance guidelines) dropped in favour of a page-by-page re-audit. **Working principles moved into `CLAUDE.md`.** Records two still-open stale code comments. |
| `docs_site_audit.md` | 2026-06-09 | Audit of the public docs site against `develop`, then the queue of content additions it turned into. Records what each page got wrong before the 2026-06 reconciliation, and why `library_base.md` and `genesis.id.md` were deleted rather than fixed. | The published site: `txdocs/`, `ledgerdocs/`, `overview/`, `multistate/`, `participate/` on `lunfardo314.github.io` `main`. Live tracker is `kb/archive/shipped/docs.md`. |
...
| `genesis_controller_signature.md` | 2026-09-08 | Genesis controller constant became a signature of description and genesis time, carrying the public key, so the key claims the library hash. Hardfork. | `constGenesisControllerSignature` in `ledger/def/def_constants0.json`; `bf490679` |
...
| `kb_reorg.md` | 2026-08-25 | The plan this knowledge base was reorganized by: 107 files classified and cut down to a live set, a research bucket and three archive buckets. | Executed in full. Kept for the *Inventory* classification and the *Progress* record; it describes the directory under its old name, `claude/`. |
...
| `lock_delegation_audit.md` | 2026-08-26 | Audit: conditional locks should call `sigLock`/`chainLock` rather than hand-roll the equivalent test. One finding, fixed in `timelock.easyfl`. | `ledger/def/timelock.easyfl` calls `_sigLock($0)`. Records the produce-side checks deliberately left alone |
...
| `monitor.md` | 2026-08-07 | Spec 0 for the monitor page: every figure it shows and where each one comes from, in three freshness tiers. | `api/monitor/`, serving live (`9996b0f2`). Spec 1 was never written — the prototype answered the open questions and section 8 records what it settled. |
...
| `network_rtt_mapping.md` | 2026-06-20 | Peer round-trip-time mapping, a distance metric, and visualization. | `api/server/netviz.go`. **Layers 1–3 shipped; the offline Monte-Carlo simulator was never built.** Companion to `kb/research/tick_duration.md`, which needs its `d(i,j)` graph. |
...
| `sequencer.md` | 2026-03-18 | The TSF sequencer design: the greedy-token-holder model, the constraints, and the incremental refactor that produced `sequencer/factory`. | `sequencer/factory/` (TransactionSkeletonFactory). Absorbed into `sequencer/README.md` (2026-08-24); still extended by `kb/sequencer_conflict_resolution.md`. |
| `sequencer_conflict_resolution.md` | 2026-08-17 | Why a sequencer must revert own state to fold in a peer that consolidated a conflicting tag-along, and the numSeq branch deferral. Measured with a conflict spammer: the search reverts freely (65% of milestones orphaned) and what breaks is endorsement breadth, collapsing 13-fold. | `sequencer/strategy_async.go` (`betterBranch`), `sequencer/branch_deferral_test.go`. Records **three widening attempts that were reverted**, and four questions still open under "Genuinely open". |
...
- **`delegate_lock.md`** and **`delegation_epoch_params.md`** — delegation has
  been de-parametrized since; `kb/research/delegation_scalability.md` is current.
