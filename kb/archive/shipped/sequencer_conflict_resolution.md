...
> **SHIPPED** — how sequencers resolve conflicting tag-alongs, and the numSeq
> branch deferral that answers it. The deferral is on `develop`
> (`sequencer/strategy_async.go`, `betterBranch`, with `branch_deferral_test.go`);
> **the code is the truth.** Archived 2026-09-03.
>
> Read it for the two things the code cannot record: the **measurements** — a
> conflict spammer proves the search reverts freely, 65% of milestones orphaned,
> and what actually breaks is endorsement breadth, collapsing 13-fold — and the
> **three widening attempts that were tried and reverted**.
>
> It also carries four questions that are **still open**, listed under "Genuinely
> open" near the end: what decides an equal-coverage tie, where the conflict
> intensity threshold is, whether the endorsement collapse is reducible at all,
> and the ~5% of slots that refuse a branch on the health gate.
>
> Status of the deferral: implemented 2026-08-17 on measurements taken
> 2026-08-12. First live evidence 2026-09-03, on the fresh genesis — all five
> sequencers consolidate into every branch (`num_seq` 5 of 5, health 7/12). That
> is an uncontended four-hour-old network, so it shows the deferral does not
> misfire, not that it helps under load. The measurement that would show that is
> the conflict-spam reproduction described under "The attack, and how to
> reproduce it".

Extends the TSF design, now in
