...
> **SHIPPED** — the plan this knowledge base was reorganized by, executed in full
> and archived 2026-09-03. Kept as the record of how the directory reached its
> present shape: the *Inventory* section is the classification every move was
> driven from, and the *Progress* table records each step. It describes the
> directory under its old name, `claude/`.

Status: **COMPLETE — all phases, 0 to 6, done (2026-08-25).** The directory came
down from 107 files to a small live set with **nothing left queued**, plus a
research bucket and three indexed archive buckets. The docs site has an ordered
four-page spine with the rest demoted to reference behind it.

**Three structural changes since**, all after this plan closed:

1. **`TODO.md` deleted** (2026-08-26) — reviewed and found stale beyond use.
2. **`research/` split out** (2026-08-26) — the documents that were investigated
   and not built now sit in their own bucket with their own index, because a
   design note beside live documents gets read as a description of how things
   work.
3. **The directory was renamed `claude/` → `kb/`** (2026-09-03), together with an
   audit that refreshed every live document's status against `develop` and a
   running node. Every reference in the repository was repointed; `.claude/`, the
   settings directory, is unrelated and untouched.

The tables further down record the directory as it was *during* the
reorganization, under its old name, and are not updated for any of the three.

What this plan does *not* cover, and what continues after it: the items in
`kb/archive/shipped/docs.md` under "What is left". All but the last are done — what remains is a
document setting the maintenance guidelines.
...
As of 2026-08-24, before this effort, `kb/` held 107 `.md` files, 1.7 MB,
...
code comment. `CLAUDE.md`'s `kb/` index lists 4 of the 107. Half the
...
1. **No repo `docs/` folder.** `docs/api.md` and `docs/txapi.md` move into the
   ...
   from this effort.
2. **Archive is `kb/archive/`** — in-repo, public, greppable, history
...
`kb/archive/shipped/docs_site_audit.md`) reconciled all of it with
`develop`; its Phase 3 additions were absorbed by this effort and completed on
2026-08-25.

Material in `kb/` that belongs to an *end user or token holder* and is not
on the site yet:
...
| Source | Destination |
|--------|-------------|
| `inflation.md` | ✅ done 2026-08-25 — the user-facing half went to `overview/2-tokens-and-supply.md` (economics) and `overview/4-transactions.md` (mechanics). *(This row originally said `overview/incentives.md`; that page has since been retired.)* `inflation.md` **stays** in `kb/`: the arithmetic, closed forms, overflow analysis and emulation tool are not public material |
| `delegation_scalability.md`, `delegation_freeze_distribution.md` | the conceptual half → `overview/delegation.md` |
| `tick_duration.md` | ✅ done 2026-08-25 — the conceptual half is `overview/consensus.md` § "How long a slot is, and why". The measurement and the model **stay** in `kb/`, now marked RESEARCH: the tick question is still open |

Phase-3 items 3 and 5 (single-signature model / holder ID) were site-only edits
with no `kb/` source; both landed in `txdocs/tx.md` on 2026-08-25.
...
the user without an answer; `kb/research/forced_delegation.md` is research only,
...
Rewritten from `kb/` sources, each against `develop`, one at a time:

| New doc | Sources |
|---------|---------|
| `core/README.md` — DAG / memDAG / attacher model, the readable way in to `dag_semantics.md` | `dag_semantics.md` (which stays in `kb/` as the hard constraint) |
| `core/core_modules/forward_sync/sync.md` — how a node catches up, the readable way in to `sync_semantics.md` | `sync.md` (which is archived after); `sync_semantics.md` stays in `kb/` as the hard constraint and is only linked, never absorbed |
...
`kb/` shrinks to the working set: documents that are *load-bearing for the
...
A `kb/` source that has been fully absorbed into a site page is archived,
...
`kb/` file; rewrite the `kb/` index table in `CLAUDE.md` to cover all
of them.
...
6. Repo-side: archive the absorbed `kb/` sources, repoint links to site
URLs.
...
Five further items are queued in `kb/archive/shipped/docs.md` under "What is left": two new
...
| 1 | move | `git mv` → `kb/archive/{incidents,shipped,superseded}/` | ✅ | 2026-08-24 — 71 files: 18 / 36 / 17 |
...
| 2 | index | status headers on surviving `kb/` files | ✅ | 2026-08-24 — all 35; `network_connectivity.md` body status corrected |
| 2 | index | rewrite `kb/` index table in `CLAUDE.md` | ✅ | 2026-08-24 — 15 keep rows + archive buckets; `sync_semantics.md` added to the core paragraph |
...
**The archive move is not purely mechanical.** 140 references to `kb/*.md`
...
pkg 14 · incidents 18 · shipped 42 · superseded 11. So `kb/` ends at 13
