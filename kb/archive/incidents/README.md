...
(`kb/dag_semantics.md`, `kb/sync_semantics.md`) do. These notes are
...
| `delegation_freeze_stall.md` | 2026-08-26 | Consensus health decayed while capital participation stayed full. The delegation pool is fed by the sequencer's own milestones, so a delegation transitioned by its *master* — a top-up or re-target — was invisible to it; the entry kept a dangling outputID and silently stopped being frozen. | Fixed `3ecdb614`. Validated on the fresh genesis 2026-09-03: all 24 delegations frozen, symptom not recurring. Read before touching `sequencer/delegationpool`. |
