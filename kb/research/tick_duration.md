...
flat-branch-inflation work (`kb/inflation.md`). Any tick change should be paired with
...
- **Partition.** If a partition splits capital so no side holds $C_h M$ within $R(T)$,
  ...
  suppressible health gate enforces; see `kb/…branch_health`). A longer slot tolerates
