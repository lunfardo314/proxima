...
> **RESEARCH** — how the number of delegations drives permanent state growth,
> and the fixed freeze grid that answers it.
> **Binds:** `ledger/def/lock_delegate.easyfl`
>
> **Partial exception to this directory's rule.** Everything else in `research/`
> is unbuilt; here the *mechanism* of §8 and §9 shipped 2026-08-15 and is live —
> the epoch grid and freeze depth are ledger constants, readable off a running
> node as `delegation_epoch_slots: 600` and `delegation_max_frozen_epochs: 60`,
> and they took effect at the 2026-09-03 genesis reset.
>
> What is research is **the load model itself, which is measured against
> nothing.** Written for the fair-launch plan, which assumes mined capital gets
> delegated. Every parameter is read from the code and every conclusion is
> arithmetic on it; not one of the load figures has been observed on a running
> network. The measurements that would settle it are listed in §10, and the
> reset restarted the delegation population from zero, so they are only now
> becoming possible.
>
> Treat the arithmetic as a hypothesis about a network that has not yet existed
> at the size it models. Archived here 2026-09-03.
