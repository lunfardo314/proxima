...
> here 2026-08-25, and flagged inline where they occurred): $M_0$ was said to derive from
...
$$\text{chainInflationOneSlot}(A,s)=\frac{A}{M_0+s}$$

where $A$ is the token amount and

$$M_0=\text{minimumInflatableAmount0}=\frac{\text{constTargetBaseSupply}}{\text{constSlotInflationBase}}.$$

With default parameters, $M_0 = 10^{15}/(33\times 10^{6}) = 30{,}303{,}030$.
...
genesis mint — six per cent of `constTargetBaseSupply` — so recomputing $M_0$ from it gives
...
relative to $M_0$. The decline is gradual: ~9.2% at year 1, ~8.5% at year 2, ~7.7% at year 3.
...
$$\text{chainInflationMultiStep}(A,s,N)=\frac{N\,A}{M_0+s}$$

Each slot produces exactly $A/(M_0+s)$ of chain inflation regardless of intermediate
compounding. After $k$ slots the amount is $A\,(M_0+s+k)/(M_0+s)$, and

$$\frac{A\,(M_0+s+k)}{(M_0+s)\,(M_0+s+k)}=\frac{A}{M_0+s},$$

so the per-slot inflation is constant. Key identity:

$$A+\frac{N A}{M_0+s}\;=\;(A+I)+\frac{(N-1)(A+I)}{M_0+s+1},\qquad I=\frac{A}{M_0+s}.$$

The final inflated amount is independent of step decomposition.

This allows computing inflation over arbitrarily large periods (months, years) in O(1)
without iterating slot by slot ($O(1)$ rather than $O(N)$).
...
When the branch bonus $B$ is constant over a segment, the slot-by-slot recurrence

$$\text{supply}_{k+1}=\text{supply}_k\cdot\frac{M_0+s+k+1}{M_0+s+k}+B$$

has the exact solution

$$\text{supply}_S=A\cdot\frac{M_0+s+S}{M_0+s}+B\,(M_0+s+S)\,\ln\!\left(\frac{M_0+s+S}{M_0+s}\right).$$

The total inflation for the segment is:
- Chain inflation: $\dfrac{N A}{M_0+s}$ — exact, via EasyFL.
- Branch inflation: $B\,(M_0+s+S)\,\ln\!\left(\dfrac{M_0+s+S}{M_0+s}\right)$ — closed form,
  accounting for chain compounding of the branch bonuses.
...
Verified: step $=1$ against step $=30$ days over 60 days gives a relative error of
$6\times10^{-8}\%$ — 640K tokens out of $10^{15}$, pure integer rounding.
...
- `minimumInflatableAmount0`: $M_0=\text{constTargetBaseSupply}/\text{constSlotInflationBase}=30{,}303{,}030$
  (**not** `InitialSupply`, which is 6% of it — see above)
- `chainInflationOneSlot(amount, slot)`: $\text{amount}/(M_0+\text{slot})$
- `chainInflationMultiStep(amount, slot, nSlots)`: $\text{nSlots}\times\text{chainInflationOneSlot}(\text{amount},\text{slot})$
...
approaches $2^{63}\approx 9.22\times10^{18}$, the signed-interpretation boundary.

**Closed form.** With a constant inflow $r$ per slot on top of chain inflation, the supply
obeys $A'=A/(M_0+s)+r$, whose solution over a segment is

$$A(s_2)=(M_0+s_2)\left[\frac{A(s_1)}{M_0+s_1}+r\,\ln\!\left(\frac{M_0+s_2}{M_0+s_1}\right)\right].$$

Take $r=B$ (the branch bonus) after emission ends, and $r=B+R_{\text{init}}/S_{\text{mine}}$
during it.
...
|--------------|---------|---------------------|
| $5\times10^{6}$ (the base) | the largest bonus the VRF can draw. **The realistic figure** — see below | **~40,000 years** |
| $2.5\times10^{6}$ | the mean of a *single* uniform draw in $[1,\text{base}]$. Naive; not what the ledger records | **~57,000 years** |
...
Use the $5\times10^{6}$ row. The $2.5\times10^{6}$ row is kept only to show what the naive
reading costs.

Mined emission barely moves either figure: $R_{\text{init}}=9.4\times10^{14}$ motes is 0.5%
of the supply at the slot limit, though it dominates the first year.

**The binding limit is the slot counter, not the amount.** Slots are `uint32` and every
value is valid, so the last representable slot is $2^{32}-1=4{,}294{,}967{,}295$, about
**1,395 years**. At that slot the supply is $\approx 1.88\times10^{17}$ motes — 188 billion
PROX, or **2% of $2^{63}$** — leaving $49\times$ headroom.

**Conclusion**: `uint64` overflow is not a concern, and not by a small margin — the ledger's
own time representation is exhausted roughly $40\times$ sooner than the amount range.
Reaching $2^{63}$ would require both a wider slot and tens of thousands of years.
