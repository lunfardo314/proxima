...
func TestInflationConst1Year(t *testing.T) {
	...
	t.Logf("chain inflation per initial slot: %s", util.GoTh(ledger.L().ID.CalcChainInflationAmount(inTs, outTs, ledger.DefaultInitialSupply, 0)))
...
func TestInflation(t *testing.T) {
	...
	t.Run("too small amount", func(t *testing.T) {
		...
		src := fmt.Sprintf("calcChainInflationAmount(%s, %s, u64/100000, u64/0)", tsIn.Source(), tsOut.Source())
		//lib.EvalFromSource(easyfl.NewGlobalDataTracePrint(nil), src)
		ledger.L().MustEqual(src, "u64/0")
		inflationDirect := ledger.L().ID.CalcChainInflationAmount(tsIn, tsOut, 100000, 0)
	...
	t.Run("amount = fraction", func(t *testing.T) {
		...
		expectedInflation := ledger.L().ID.CalcChainInflationAmount(tsIn, tsOut, amountIn, 0)
		src := fmt.Sprintf("calcChainInflationAmount(%s, %s, u64/%d, u64/0)", tsIn.Source(), tsOut.Source(), amountIn)
		ledger.L().MustEqual(src, "u64/50")
		ledger.L().MustEqual(src, fmt.Sprintf("u64/%d", expectedInflation))

		src = fmt.Sprintf("calcChainInflationAmount(%s, %s, u64/%d, u64/0)", tsIn.Source(), tsOut.Source(), amountIn)
		ledger.L().MustEqual(src, fmt.Sprintf("u64/%d", ledger.L().ID.CalcChainInflationAmount(tsIn, tsOut, amountIn, 0)))
		t.Logf("inflationAmount: %s", util.GoTh(ledger.L().ID.CalcChainInflationAmount(tsIn, tsOut, amountIn, 0)))
