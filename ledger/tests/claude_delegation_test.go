...
func setupDelegEnv(t *testing.T, maxFrozenEpochs byte, inflationShare uint16) *delegTestEnv {
	...
	delegOut := ledger.MakeDelegationInitOutput(ledger.MakeDelegateInitOutputParams{
		Amount:                 cdelegTokens,
		MasterID:               base.SpenderID(env.masterAddr),
		Target:                 env.target,
...
func TestClaudeDelegationOriginCannotBeFrozen(t *testing.T) {
	...
	delegLock := ledger.NewDelegateLock(target, base.SpenderID(masterAddr), 4, 0)
...
func TestClaudeDelegationInflationShareAbove1000(t *testing.T) {
	...
	delegOut := ledger.MakeDelegationInitOutput(ledger.MakeDelegateInitOutputParams{
		Amount:                 cdelegTokens,
		MasterID:               base.SpenderID(masterAddr),
		Target:                 target,
		MaxFrozenEpochs:        4,
...
func TestClaudeDelegationOnHoldTargetRelock(t *testing.T) {
	...
	successorChainConstraint2 := ledger.NewChainConstraint(env.seqChainOrigin.ChainID, 0, env.seqChainOrigin.OriginSlot, 0, 0, env.seqChainOrigin.TransitionCounter+1)
	...
	cc := ledger.NewChainConstraint(env.delegatedOutput.ChainID, predIdx2, env.delegatedOutput.OriginSlot, 0, 0, env.delegatedOutput.TransitionCounter+1)
