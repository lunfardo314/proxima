...
func TestPrintAllForms(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Logf("--- LedgerTime forms (%s) ---", tc.name)
			t.Logf("  String (==dashed):                 %s", tc.ts.String())
			t.Logf("  Short (==dashed):                  %s", tc.ts.Short())
			t.Logf("  AsFileName (==String):             %s", tc.ts.AsFileName())
			t.Logf("  StringLegacy:                      %s", tc.ts.StringLegacy())
			t.Logf("  ShortLegacy:                       %s", tc.ts.ShortLegacy())
			t.Logf("  AsFileNameLegacy:                  %s", tc.ts.AsFileNameLegacy())

			// LedgerTime dashed-form invariants
			require.Equal(t, tc.ts.String(), tc.ts.AsFileName())
			require.NotContains(t, tc.ts.String(), "|")
			require.NotContains(t, tc.ts.Short(), "|")
			require.NotContains(t, tc.ts.AsFileName(), "_")

			t.Logf("--- TransactionID forms (%s) ---", tc.name)
			t.Logf("  String (==StringDashed):           %s", txid.String())
			t.Logf("  StringShort (==StringDashedShort): %s", txid.StringShort())
			t.Logf("  StringVeryShort (==...VeryShort):  %s", txid.StringVeryShort())
			...
			t.Logf("  StringLegacy:                      %s", txid.StringLegacy())
			t.Logf("  StringLegacyShort:                 %s", txid.StringLegacyShort())
			t.Logf("  StringLegacyVeryShort:             %s", txid.StringLegacyVeryShort())
			t.Logf("  AsFileName (==StringDashed):       %s", txid.AsFileName())
			t.Logf("  AsFileNameShort:                   %s", txid.AsFileNameShort())
			t.Logf("  AsFileNameLegacy:                  %s", txid.AsFileNameLegacy())
			t.Logf("  AsFileNameLegacyShort:             %s", txid.AsFileNameLegacyShort())

			// AsFileName delegates to the dashed full form; the short form has no ".." (Windows-safe).
			require.Equal(t, txid.StringDashed(), txid.AsFileName())
			require.NotContains(t, txid.AsFileNameShort(), "..")

			// String/StringShort/StringVeryShort delegate to the dashed equivalents.
			require.Equal(t, txid.StringDashed(), txid.String())
			require.Equal(t, txid.StringDashedShort(), txid.StringShort())
			require.Equal(t, txid.StringDashedVeryShort(), txid.StringVeryShort())

			t.Logf("--- OutputID forms (idx=%d) ---", oid.Index())
			t.Logf("  String (==StringDashed):           %s", oid.String())
			t.Logf("  StringShort (==StringDashedShort): %s", oid.StringShort())
			t.Logf("  StringVeryShort (==...VeryShort):  %s", oid.StringVeryShort())
			...
			t.Logf("  StringLegacy:                      %s", oid.StringLegacy())
			t.Logf("  StringLegacyShort:                 %s", oid.StringLegacyShort())
			t.Logf("  StringLegacyVeryShort:             %s", oid.StringLegacyVeryShort())

			require.Equal(t, oid.StringDashed(), oid.String())
			require.Equal(t, oid.StringDashedShort(), oid.StringShort())
			require.Equal(t, oid.StringDashedVeryShort(), oid.StringVeryShort())
		})
	}
}
