...
// for Hash + GenesisControllerSignature. The signature must be a real one
// over the description and genesis time: UnmarshalJSON verifies it.
func TestConstants_JSONRoundTrip(t *testing.T) {
	...
	sig := base.SignatureDataED25519(priv, txbuildercore.GenesisControllerSignedMessage("test ledger", 1_700_000_000))

	orig := &txbuildercore.Constants{
		Hash:                        hashBytes,
		Description:                 "test ledger",
		GenesisControllerSignature:  sig,
	...
	// Hash and signature are plain hex strings (no 0x prefix).
	require.Contains(t, s, `"hash":"`+hex.EncodeToString(hashBytes[:])+`"`)
	require.Contains(t, s, `"genesis_controller_signature":"`+hex.EncodeToString(sig)+`"`)
	...
	require.True(t, got.GenesisControllerPublicKey().Equal(priv.Public()))

	// a signature over a different description does not verify
	bad := strings.Replace(s, `"description":"test ledger"`, `"description":"other ledger"`, 1)
	require.Error(t, json.Unmarshal([]byte(bad), &got))
}
