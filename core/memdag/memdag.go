...
//	lrb, atDepth := multistate.CheckTransactionInLRB(d.StateStore(), txid, maxDepth)
...
func (d *MemDAG) LatestBranchSlots() (slot, healthySlot uint32, synced bool) {
	...
	if d.latestHealthyBranchSlot == 0 {
		healthyExists := false
		d.latestHealthyBranchSlot, healthyExists = multistate.FindLatestHealthySlot(d.StateStore())
