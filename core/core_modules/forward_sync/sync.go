...
const (
	...
	// matches half the branch txID retention (kb/archive/shipped/txid_ttl_tiered.md), the depth to which branch
...
func Start(env environment) *Sync {
	...
	if len(sourceURLs) == 0 {
		...
		// bootstrap / standalone configuration. See kb/archive/incidents/fork_detection_recovery.md.
...
// sequencer gate does not block on this. See kb/archive/incidents/fork_detection_recovery.md §1, §3.
...
// pruned past the fork point). See kb/archive/incidents/fork_detection_recovery.md §2b.
//
...
func (s *Sync) syncTick() {
	...
	// target set is a draining quantity, so a single level test cannot flap. See kb/sync_semantics.md.
