# TagAlongLock: Change $1 from SigLock bytecode to raw SpenderID

## Motivation

The TagAlongLock EasyFL constraint previously stored its sender as full SigLock bytecode
(`sigLock(0x<spenderID>)`, i.e. `a(0x<hash>)`). This added unnecessary complexity:

- **Production validation** had to parse bytecode to extract the spender hash for comparison
  (`parseInlineDataArgument(...)` to get the hash, then compare with `txSpenderID`)
- **Consumption (reclaim)** evaluated `$1` as a constraint directly, coupling the lock format
  to the validation logic

Storing a raw 32-byte spender ID is simpler and more direct.
...
**After:**
- `$1` is now raw 32-byte spender ID
- Production: `equal($1, txSpenderID(txSignatureData))` -- direct comparison
- Reclaim: `sigLock($1)` -- wraps raw ID into sigLock constraint at validation time

### Go side (`ledger/lock_tag_along.go`)

- Struct field: `Sender SigLock` -> `SenderID base.SpenderID`
...
- `NewTagAlongOutput()`: parameter `sender SigLock` -> `senderID base.SpenderID`
...
- `req_seqdata.go`, `req_withdraw.go`, `req_askstop.go`: `Sender: sender` -> `SenderID: base.SpenderID(sender)`
...
All wrap the `SigLock` value in `base.SpenderID(...)` cast since `SigLock` is `type SigLock base.SpenderID`.
...
1. **No semantic change**: The EasyFL constraint enforces identical rules. Production still
   verifies `$1 == txSpenderID(txSignatureData)`. Reclaim still requires sigLock match.
...
3. **`SigLock` and `SpenderID` are the same underlying type** (`[32]byte`). The cast is
   zero-cost and preserves all existing authorization checks in the sequencer builder
   (sender ID comparison against `SpenderIDFromPublicKey`).
