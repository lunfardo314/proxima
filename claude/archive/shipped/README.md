...
| `network_rtt_mapping.md` | 2026-06-20 | Peer round-trip-time mapping, a distance metric, and visualization. | `api/server/netviz.go`. **Layers 1–3 shipped; the offline Monte-Carlo simulator was never built.** Companion to `claude/research/tick_duration.md`, which needs its `d(i,j)` graph. |
