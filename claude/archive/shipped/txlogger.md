# Transaction logger (txlogger) — original spec

> **SHIPPED** — the full specification, requirements, phase plan and
> as-built file list, archived from `txlogger/README.md` on 2026-08-26 when that
> README was trimmed to a reference. The code is the truth; this records what
> was intended, the requirements that shaped it (the RocksDB-migration
> abstraction, manual TTL rather than Badger's native one) and the build order.
