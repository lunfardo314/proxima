...
type (
	...
	StartStop interface {
		...
		Stop()
		// GracefulShutdown initiates orderly node shutdown with a reason logged prominently and saves
		// a crash log. Callable from any context (detached vertex, signal handler, memory watchdog,
		// etc.). Idempotent — safe to call multiple times from different goroutines.
		GracefulShutdown(reason string)
		// GracefulShutdownNoCrashLog is GracefulShutdown without saving a crash log, for intentional
		// operator-initiated shutdowns (e.g. SIGINT / Ctrl-C).
		GracefulShutdownNoCrashLog(reason string)
		IsShuttingDown() bool
	...
	NodeGlobal interface {
		...
		// only by the operational network-mapping overlay (see peering/network_connectivity.md).
