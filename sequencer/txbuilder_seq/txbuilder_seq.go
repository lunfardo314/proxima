...
func (txb *SeqTxBuilder) FreezeDelegation(delegationIn *ledger.DelegationOutput) error {
	if delegationIn.Target.ChainID() != txb.chainInput.ChainID || !delegationIn.IsUnlockableByTarget(txb.TransactionData.Timestamp.Slot) {
	...
	lastEpochToFreeze, frozenEpochs := delegationIn.LatestPossibleEpochToFreeze(txb.TransactionData.Timestamp)
	predIdx := byte(len(txb.ConsumedOutputs))
	delegationOut, projectedInflation, err := delegationIn.MakeDelegationFreezeOutput(ledger.MakeDelegationSuccessorOutputParams{
	...
	inflation := delegationOut.Inflation()
	requiredAdvanceByOutput := delegationOut.TokenBalance() - delegationIn.Output.TokenBalance() - inflation

	// TODO

	if requiredAdvanceByOutput > projectedInflation-txb.SequencerData.InflationMargin(projectedInflation) {

	}

	// calculate projected inflation from the POV of the sequencer
	projectedInflation := delegationIn.ProjectedInflation(txb.TransactionData.Timestamp, byte(frozenEpochs))
	// charge inflation margin as defined in the sequencer data profile
	breakEvenAdvance := projectedInflation - txb.SequencerData.InflationMargin(projectedInflation)
	if requiredAdvanceByOutput > breakEvenAdvance {
		return fmt.Errorf("SeqTxBuilder.FreezeDelegation: advance required by the delegation output is goo big")
	}

	// inflation generated by this step
	inflation := ledger.L().CalcChainInflationAmountOneSlot(delegationIn.ID.Slot(), delegationIn.Output.TokenBalance())

	successorTokenBalance := txb.chainOutAmounts[ledger.AmountIndexTokenBalance] + int64(inflation)
	if successorTokenBalance-int64(breakEvenAdvance) < int64(ledger.L().ID.MinimumAmountOnSequencer) {
	...
	txb.chainOutAmounts[ledger.AmountIndexTokenBalance] -= int64(breakEvenAdvance)
