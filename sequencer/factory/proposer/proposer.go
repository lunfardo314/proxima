...
type (
	Environment interface {
		...
		GetLatestMilestone() vertex.WrappedOutput
		StoreProposalDuration(d time.Duration)
...
func (c *TaskGeneric) Name() string {
	return fmt.Sprintf("%s-%s", c.Strategy.Name, c.TargetTs.String())
}
...
func (c *TaskGeneric) RunAndIgnoreDeletedVerticesException(fun func()) {
...
func (c *TaskGeneric) StoreProposalDuration() {
	c.Env.StoreProposalDuration(time.Since(c.startTime))
}
