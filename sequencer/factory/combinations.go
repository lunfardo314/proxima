...
// attacher work. Hence the mutex — it is read and written from all of their goroutines.
...
// checkAndMark reports whether the combination was already checked, marking it if not, in one
// step. Two factories racing the same combination would otherwise both see it unchecked and both
// build it, which is the duplicated work the shared set exists to prevent.
...
// can be retried later in the slot instead of being written off for the whole of it.
